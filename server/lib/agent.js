...
import type { THullMetrics, THullClientLogger } from "./types";
 ...
 */
function calculateUpdatedSinceTimestamp(
  connector: any,
  safetyInterval: number,
  nowUnix: number
): Date {
  let lastSyncAt = parseInt(
    _.get(connector, "private_settings.last_sync_at"),
    10
  );
...
class Agent {
  ...
  constructor(hullClient: any, connector: any, metricClient: IMetricsClient) {
    ...
    this.leadFieldsInbound = _.uniq(
      ["id"].concat(
        connector.private_settings.lead_attributes_inbound || [
          "id",
          "name",
          "url"
        ]
    ...
    this.leadFieldsInbound = _.uniq(
      this.leadFieldsInbound.concat(["name", "url"])
    );
    ...
    this.closeClient.listCustomFields(100, 0).then(
      cf => {
  ...
  fetchUpdatedLeads(): Promise<any[]> {
    ...
    const sinceTimestamp = calculateUpdatedSinceTimestamp(
      this.connector,
      6000,
      nowUnix
    );
    ...
    const data = [];
    return this.closeClient.listLeads(q, this.leadFieldsInbound, 100, 0).then(
      res => {
        data.push(res.data);
        if (res.has_more === true) {
          ...
          for (let index = 1; index < totalPages; index++) {
            ...
            apiOps.push(
              this.closeClient.listLeads(q, this.leadFieldsInbound, 100, index)
            );
          }
  ...
  sendUserMessages(messages: any[]): Promise<any> {
    ...
    const apiOps = _.map(envelopes, e => {
      const q = `lead_url in ("${e.message.account.domain}", "http://www.${
        e.message.account.domain
      }", "https://${e.message.account.domain}", "https://www.${
        e.message.account.domain
      }")`;
      return this.closeClient
        .listLeads(q, ["id", "url", "name", "contacts"])
        .then(leads => {
    ...
    return Promise.all(apiOps).then(() => {
      ...
      _.forEach(leadsFilterResult.toSkip, e => {
        this.hullClient
          .asAccount(e.message.user.account)
      ...
      _.forEach(leadsFilterResult.toInsert, e => {
        const cioObj = this.attributesMapper.mapToServiceObject(
          "Lead",
          e.message.user
        );
      ...
      return Promise.map(leadCreationOps, r => {
        const traitsObj = this.attributesMapper.mapToHullAttributeObject(
          "Lead",
          r
        );
        ...
        if (r.contacts && r.contacts.length === 1) {
          const userTraits = this.attributesMapper.mapToHullAttributeObject(
            "Contact",
            r.contacts[0]
          );
        ...
        .map(leadUpdateOps, r => {
          ...
          const identObj = this.attributesMapper.mapToHullIdentObject(
            "Lead",
            r
          );
  ...
  handleFetchResults(sObjects: Array<Object>): void {
    _.forEach(sObjects, sObject => {
      ...
      this.hullClient
        .asAccount(identObj)
        .logger.info("incoming.account.success", { data: sObject });
    });
